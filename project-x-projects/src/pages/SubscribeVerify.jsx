...
export default function SubscribeVerify() {
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      try {
        const res = await fetch(`${API_BASE}/api/subscribers/verify`, {
          method: "POST",
        ...
        setState({
          loading: false,
          ok: !!json.ok,
          msg: json.message || (json.ok ? "Subscription confirmed." : "Invalid or expired link."),
        });
      } catch {
        if (cancelled) return;
        setState({ loading: false, ok: false, msg: "Couldn’t verify right now. Please try again later." });
      }
    }
        ...
        <section className="heroCard" style={{ maxWidth: 880, margin: "0 auto" }}>
          <div className="heroLeft" style={{ width: "100%" }}>
            ...
            <h1 className="h1" style={{ marginTop: 8 }}>
              {state.loading ? "Confirming…" : state.ok ? "You're subscribed" : "Couldn’t confirm"}
            </h1>
